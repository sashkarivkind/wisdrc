...
# I modified the code for my own needs Alexander Rivkind
...
def random_ccm():
  ...
  rgb2xyz = tf.compat.v1.to_float([[0.4124564, 0.3575761, 0.1804375],
...
def random_gains(enable_random_gains=False,**kwargs):
  """Generates random gains for brightening and white balance."""
  # RGB gain represents brightening.
  if enable_random_gains:
    ...
    rgb_gain = 1.0 / tf.random.normal((), mean=0.8, stddev=0.1e-10)
    # Red and blue gains represent white balance.
    red_gain = tf.random.uniform((), 2.1, 2.100000000001)
    blue_gain = tf.random.uniform((), 1.7, 1.700000000001)
...
def unprocess(image, do_mosaic=True,**kwargs):
  ...
  if True:
    ...
    rgb_gain, red_gain, blue_gain = random_gains(**kwargs)

    # Approximately inverts global tone mapping.
    image = inverse_smoothstep(image)
    # Inverts gamma compression.
    image = gamma_expansion(image) #before single to burst
    ...
    # here single to burst should occur
    if do_mosaic:
      image = mosaic(image)
...
#####
def unprocess_debug(image, do_mosaic=True,unprocess_steps={},**kwargs):
  """Unprocesses an image from sRGB to realistic raw data."""
  # with tf.name_scope(None, 'unprocess'):
  if True:
    image.shape.assert_is_compatible_with([None, None, 3])
    ...
    if 1 in unprocess_steps:
      # Approximately inverts global tone mapping.
      image = inverse_smoothstep(image)
    if 2 in unprocess_steps:
      # Inverts gamma compression.
      image = gamma_expansion(image)
    if 3 in unprocess_steps:
      # Inverts color correction.
      image = apply_ccm(image, rgb2cam)
    if 4 in unprocess_steps:
      # Approximately inverts white balance and brightening.
      image = safe_invert_gains(image, rgb_gain, red_gain, blue_gain)
    # Clips saturated pixels.
    if 5 in unprocess_steps:
      image = tf.clip_by_value(image, 0.0, 1.0)
      # Applies a Bayer mosaic.
    if do_mosaic:
      image = mosaic(image)
